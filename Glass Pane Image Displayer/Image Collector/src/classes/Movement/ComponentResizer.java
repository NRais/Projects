...
public class ComponentResizer extends MouseAdapter
{
	...
	private Dimension maximumSize = MAXIMUM_SIZE;
        ...
        private ImageWindowObject thisWindow = null;
        
	/**
	 *  Convenience contructor. 
         * @param currentWindow  - a window object to be maniupulated
	 */
	public ComponentResizer(ImageWindowObject currentWindow)
	{
		this(new Insets(5, 5, 5, 5), new Dimension(1, 1));
                
                this.thisWindow = currentWindow;
                System.out.println("Adding current window " + thisWindow);
	...
	public void mouseMoved(MouseEvent e)
	{
		...
		{  
                    // HANDLE ILLEGAL CURSOR EXCEPTION
                    if (cursors.containsKey( direction )) {
			...
			source.setCursor( cursor );
                        ...
                        direction = 12; //just put a valid cursor in instead
	...
	protected void changeBounds(Component source, int direction, Rectangle bounds, Point pressed, Point current)
	{
		...
		if (WEST == (direction & WEST))
		{
			...
			x -= drag;
			width += drag;
                        height += ifAspectIncrease(drag, tryToGetAspectRatioY()); // NOTE: we are setting the width to the drag*the Y ratio
		...
		if (NORTH == (direction & NORTH))
		{
			...
			y -= drag;
			height += drag;
                        width += ifAspectIncrease(drag, tryToGetAspectRatioX()); // NOTE: we are setting the width to the drag*the X ratio
		}
		...
		if (EAST == (direction & EAST))
		{
			...
			drag = getDragBounded(drag, snapSize.width, width, minimumSize.width, maximum);
			width += drag;
                        height += ifAspectIncrease(drag, tryToGetAspectRatioY()); // NOTE: we are setting the height to the drag*the Y ratio
		}
		...
		source.validate();
                ...
                if (thisWindow != null) {
                    thisWindow.windowSize = new Dimension(width, height);
                                    
                    ImageManipulator.resizeWindowBackground(thisWindow);
                    
                }
	}
        
        // IF we are increasing the image and retaining the aspect ration
        // then when we resize one side we want to proportionally increase the other as well otherwise it looks bad
        
        // SO FIRST: check if we are maintaining aspect
        /// (IF YES) THEN:
        /// RETURN an integer of the value of the increase for the other side
        ///
        /// This is determined by the current side increase * the size the other side is proportionally bigger
        ///
        //// (ex. change = 14px in the Y-Direction, ratio is X:Y 3:2 so X is 3/2 * Y so 14*3/2 so 21px in the x direction)
        private int ifAspectIncrease(int change, double ratioOfTheOtherSide) {
            
            int otherDirectionChange = 0;
            
            // unsuprisingly we only try to do this if it is not null
            // NULL = not resizing a frame but instead another type of object
            if (thisWindow != null) {
            
                // if maintaining aspec then resize the frame as well as the window
                if (thisWindow.maintainAspect) {

                    //** System.out.println("ASPECT: " + ratioOfTheOtherSide);

                    otherDirectionChange = (int) ((double) change * ratioOfTheOtherSide);

                    //** System.out.println("THIS: " + change + ", OTHER: " + otherDirectionChange );

                }
            }
            return otherDirectionChange;
        }
        
        // METHOD TO CHECK WHETHER TO RUN THE RESIZING LISTENERS
        private boolean resizable() {
            // the only time we don't resize is if the window is not focusable
            boolean resizable = true;
            // but before that we have to check there actually is a window (some methods have no window)
            if (thisWindow != null) {
                // ...
                if (!thisWindow.windowFrame.isFocusable()) {
                    resizable = false; // okay don't resize because the window that exists is off
                }
            }
        ...
        // ---------
        // methods to account for when non-jframe entities are resized
        // this prevents null pointer exceptions by catching them
        private double tryToGetAspectRatioX() {
            if (thisWindow != null) {
                return thisWindow.getAspectRatioX();
            } else {
                System.out.println("Resizing a non-jframe component");
                return 0;
            }
        }
        private double tryToGetAspectRatioY() {
            if (thisWindow != null) {
                return thisWindow.getAspectRatioY();
            } else {
                System.out.println("Resizing a non-jframe component");
                return 0;
            }
